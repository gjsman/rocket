        ...
        <div class="col-span-1 h-fit">
            ...
            <x-panel class="mb-6">
                <nav class="space-y-1" aria-label="Sidebar">
                    ...
                    <x-sidebar-item href="{{ route('quiz.all', ['element' => $element]) }}" :active="true">
                        {{ __('All Submissions') }}
                    </x-sidebar-item>
                </nav>
            </x-panel>
        ...
        <div class="col-span-2 h-fit">
            <x-list class="col-span-1 h-fit mb-8 md:mb-0">
                ...
                @forelse($element->submissions->sortByDesc('created_at') as $key => $submission)
                    ...
                    @endphp
                    <x-list-clickable-item href="{{ route('quiz.showSubmission', ['element' => $submission]) }}" :badge="$badge">
                        <x-slot name="title">
                ...
                @empty
                    <x-list-clickable-item title="{{ __('No submissions') }}" href="#" badge="">
                        <x-slot name="footerLeftIcons">
                            <p class="text-sm">{{ __('There are no submissions for the quiz.') }}</p>
                        </x-slot>
                    </x-list-clickable-item>
                @endforelse
            </x-list>
