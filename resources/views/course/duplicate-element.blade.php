<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Duplicate ').$element->name.__(' to a different section') }}
        </h2>
    </x-slot>
    <x-panel>
        <x-slot name="header">
        ...
        <form method="POST" action="{{ route('element.duplicate', ['course' => $element->section->course, 'class' => class_basename($element), 'id' => $element->id]) }}">
            @csrf
            <div>
                <label class="text-base font-medium text-gray-900">{{ __('Select a section') }}</label>
                <p class="text-sm leading-5 text-gray-500">{{ __('Which section should ').$element->name.__(' be duplicated to?') }}</p>
                <fieldset class="mt-4">
